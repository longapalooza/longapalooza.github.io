var MainMenu=function(){
  ...
  var mask='<div id="menu-top-mask" style="height: 2px; background-color: #fff;\
            z-index:1001;"/>';
  var timeOut;
  this.init=function(p){
    ...
    $('ul.main-menu > li > ul li').click(function(event){
      ...
      // If item is active, check if there are submenus (ul elements inside
      // current li)
    ...
    toggleMenuItem=function(el){
      ...
      var top=parseInt(el.css('padding-bottom').replace("px", ""), 10)+
              parseInt(el.css('padding-top').replace("px", ""), 10)+
              el.position().top+el.height();
      ...
      $mask.css({position: 'absolute', top: '-1px', width: (maskWidth)+'px'});
    ...
    toggleSubMenu=function(el){
      if(el.hasClass(settings.disabledClass)){return;}
      var submenu=el.find("ul:first");
